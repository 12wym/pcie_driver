...
#define DRIVER_VERSION 2502100101
...
static irqreturn_t pcie_xdma_read_req_handler(int irq, void *dev_id)
{
	...
	{
		queueBufAddr = dmaQueueManagerHandler->queueArray[ADC_READ_QUEUE].tail * (dmaQueueManagerHandler->queueArray[ADC_READ_QUEUE].frameSize) +\
					dmaQueueManagerHandler->queueArray[ADC_READ_QUEUE].virtualAddr;//queue;
		// printk("data: ");
		// for(index = 0; index < ADC_READ_SIZE; index++)
		// {
		temp = ioread64(io_hwaddr + ADC_PCIE_ADDRESS_OFFSET);
		// printk("%016lx", temp);
		*((unsigned int *)queueBufAddr + 1) = (unsigned int)(temp & 0xFFFFFFFF);
		*((unsigned int *)queueBufAddr + 2) = (unsigned int)(temp >> 32 & 0xFFFFFFFF);
		temp = ioread64(io_hwaddr + ADC_PCIE_ADDRESS_OFFSET + 8);
		// printk("%016lx", temp);
		*((unsigned int *)queueBufAddr + 5) = (unsigned int)(temp & 0xFFFFFFFF);
		*((unsigned int *)queueBufAddr + 6) = (unsigned int)(temp >> 32 & 0xFFFFFFFF);
		temp = ioread64(io_hwaddr + ADC_PCIE_ADDRESS_OFFSET + 16);
		// printk("%016lx", temp);
		*((unsigned int *)queueBufAddr + 7) = (unsigned int)(temp & 0xFFFFFFFF);
		*((unsigned int *)queueBufAddr + 8) = (unsigned int)(temp >> 32 & 0xFFFFFFFF);
		temp = ioread64(io_hwaddr + ADC_PCIE_ADDRESS_OFFSET + 24);
		// printk("%016lx", temp);
		*((unsigned int *)queueBufAddr + 3) = (unsigned int)(temp & 0xFFFFFFFF);
		*((unsigned int *)queueBufAddr + 4) = (unsigned int)(temp >> 32 & 0xFFFFFFFF);
		temp = ioread64(io_hwaddr + ADC_PCIE_ADDRESS_OFFSET + 32);
		// printk("%016lx", temp);
			// printk("%llx ", queueBufAddr[index]);
		// }
		*((unsigned int *)queueBufAddr) = 0x28;
		// printk("\n");
		dmaQueueManagerHandler->queueArray[ADC_READ_QUEUE].tail = (dmaQueueManagerHandler->queueArray[ADC_READ_QUEUE].tail + 1) % \
...
static int dac_data_send(int frameIndex)
{
	...
	else
	{
		...
		//
		dmaQueueManagerHandler->queueArray[DAC_WRITE_QUEUE].tail = (dmaQueueManagerHandler->queueArray[DAC_WRITE_QUEUE].tail + 1) % dmaQueueManagerHandler->queueArray[DAC_WRITE_QUEUE].totalNum;
		dmaQueueManagerHandler->queueArray[DAC_WRITE_QUEUE].restIdleNum--; // = dmaQueueManagerHandler->queueArray[DAC_WRITE_QUEUE].totalNum - ((dmaQueueManagerHandler->queueArray[DAC_WRITE_QUEUE].tail - dmaQueueManagerHandler->queueArray[DAC_WRITE_QUEUE].head) % dmaQueueManagerHandler->queueArray[DAC_WRITE_QUEUE].totalNum)
		spin_unlock(&(dmaQueueManagerHandler->queueArray[DAC_WRITE_QUEUE].spinlock));
	}
	return 0;
}
...
static long pcie_dac_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case DATA_SEND:
		ret = copy_from_user(&frameIndex, (int *)arg, sizeof(frameIndex));
...
// static const struct file_operations pcie_time_fops = {
// 	.open = pcie_time_open,
// 	.release = pcie_time_release,
// 	.unlocked_ioctl = pcie_time_ioctl,
// 	// .mmap = pcie_time_mmap,
// };
...
|* FPGA_EXPIRE_NUM: %u *|\n\
|-----------------------END-----------------------|\n", \
DRIVER_VERSION, ((fpga_version >> 9) & 0x7F), ((fpga_version >> 5) & 0xF), ((fpga_version >> 0) & 0x1F), ((fpga_version >> 24) & 0xFF), ((fpga_version >> 16) & 0xFF), transfer_times, transfer_error_times, fpga_dac_expired_frame_num, fpga_dac_total_rec_frame_num, (u64)0, (u64)0, (u32)0);
