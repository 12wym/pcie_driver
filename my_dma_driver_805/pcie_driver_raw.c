	...
	if(irq == irq_msi_vec[0])
	{
		...
		else
		{
			...
			*((unsigned int *)queueBufAddr + 2) = (unsigned int)(temp >> 32 & 0xFFFFFFFF);//ns
			...
			// printk("%016lx", temp);
			*((unsigned int *)queueBufAddr + 5) = (unsigned int)(temp & 0xFFFFFFFF);//chanel 10
			*((unsigned int *)queueBufAddr + 6) = (unsigned int)(temp >> 32 & 0xFFFFFFFF);//chanel 32
			temp = ioread64(io_hwaddr + ADC_PCIE_ADDRESS_OFFSET + 16);
			// printk("%016lx", temp);
			*((unsigned int *)queueBufAddr + 7) = (unsigned int)(temp & 0xFFFFFFFF);//chanel 54
			*((unsigned int *)queueBufAddr + 8) = (unsigned int)(temp >> 32 & 0xFFFFFFFF);//chanel 76
